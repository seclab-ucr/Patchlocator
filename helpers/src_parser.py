    ...
    if funcline==0:
        ...
        for element in cur_func_inf_r:
            ...
            if funcname2==funcname and funcline2==funcline:
                ...
                result.add(content)
    return result
def parse_patch(patch,kernel,changefiles=None):
    #print 'parse_patch'
    #print patch
    #print kernel
    inf = {}
    ...
    for i in range(len(diff_index)-1):
        inf.update(_parse_patch_diff(p_buf,diff_index[i],diff_index[i+1],kernel,changefiles))
    #_adj_line_number(inf)
    return inf

src_map = {}
def _parse_patch_diff(p_buf,st,ed,kernel,changefiles):
    ...
    try:
        if changefiles:
            for (oldfile,newfile) in changefiles:
                if oldfile==fp:
                    fp=newfile
        ...
        print 'fail when try to build_func_map for',kernel,fp
        return inf
    if not s_buf:
...
def _parse_patch_at(p_buf,st,ed,s_buf):
    ...
    #if "timerfd_remove_cancel" in head:
    #dbg_out=True
    #else:
    #    dbg_out=False
    if dbg_out:
    ...
    for i in range(1,len(c_index)-1):
        ...
        Linenumber=lno+LINE_BASE
        func_name = get_func_name(Linenumber)
        if not func_name:
            if dbg_out:
...
def get_func_name(lno):
    ...
    for k in cur_func_inf:
        ...
        if lno >= k[0] and lno <= k[1]:
            #if dbg_out:
            #    print  "found the function",k[0], "  ",k[1]
