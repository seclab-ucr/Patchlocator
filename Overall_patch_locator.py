...
def Locate_patch_sourcecodesnapshot(repo,branch,patchesinfo,targetkernel):
    Patch_locator.patchlocator(repo,branch,patchesinfo)
    Patch_evolution.Patchevolution_tracker(repo,branch,patchesinfo)
    Patch_matcher_src.compare_sourcecode(branch,targetkernel)

def Locate_patch_binarysnapshot(repo,branch,patchesinfo,targetkernel):
    Patch_locator.patchlocator(repo,branch,patchesinfo)
    Patch_evolution.Patchevolution_tracker(repo,branch,patchesinfo)
...
def Locate_patch():
    ...
    targetkernel=sys.argv[5]
    if mode == 'repo':
        Locate_patch_repository(repo,branch,patchesinfo)
    elif mode == 'source':
        Locate_patch_sourcecodesnapshot(repo,branch,patchesinfo,targetkernel)
    elif mode == 'binary':
        Locate_patch_binarysnapshot(repo,branch,patchesinfo,targetkernel)
    else:
